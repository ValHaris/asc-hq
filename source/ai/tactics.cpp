         ...
         */
         do {   // while currentTarget != targets.end()
            ...
            if ( enemy ) {
               ...
               tactics_findBestAttackUnits ( attacker, mvci, positions, 0, finalPositions, finalValue, 0, 0, ASC_GetTicks() );
...
{
   if ( m == mvc.end() || unitsPositioned >= 6 || recursionDepth >= 8 || (startTime + config.maxTactTime < ASC_GetTicks() && !benchMark)) {
      float value = 0;
